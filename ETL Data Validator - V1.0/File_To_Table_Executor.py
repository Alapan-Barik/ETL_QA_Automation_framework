...
# MAGIC %md
# MAGIC This is a **plug-and-play** File -> Table regression harness - the
# MAGIC same source-to-target validation as `Executor.py`, except the
# MAGIC **source** side comes from a flat file instead of a SQL query.
# MAGIC
# MAGIC   Everything this file needs to run comes from `Data_Validator.py`
# MAGIC   (loaded below) and `File_Reader.py` (loaded below that). No check
# MAGIC   logic lives in this file - it only reads the source file into a
# MAGIC   temp view and hands off to the exact same checks `Executor.py` uses.
# MAGIC   It runs two ways:
# MAGIC   1. **In Databricks** - import all three files into a Repo/Workspace
# MAGIC      folder and "Run all". `dbutils`, `spark` and `display()` are
# MAGIC      provided by the runtime.
# MAGIC   2. **Locally / in CI** - `python File_To_Table_Executor.py` with
# MAGIC      `pyspark` installed (same local/CI story as `Executor.py`). With
# MAGIC      no source file configured it validates the built-in demo file
# MAGIC      under `sample_files/customers_source.csv` against a small demo
# MAGIC      target table so you can see every check working end to end.

# COMMAND ----------

# DBTITLE 1,Load Data_Validator and File_Reader
# MAGIC %run "/Users/alapanbarik26@gmail.com/ETL Validator/Data_Validator"

# COMMAND ----------

# MAGIC %run "/Users/alapanbarik26@gmail.com/ETL Validator/File_Reader"
...
snap_date = as_of_date   # the snapshot / partition date your target query filters on
print(f"********** File -> Table Validation run for {snap_date} **********")
...
job_id = "FILE_TO_TABLE_VALIDATION_JOB"
run_id = "local-run"
...
REPORT_TITLE = "File-to-Table Validation"  # heading on the HTML dashboard report
...
# DBTITLE 1,Source File & Target Query -- EDIT ME

#  --- Source: a flat file, parsed via File_Reader.read_file() -------------
#  Every key here is optional except file_path - see File_Reader.py's module
#  docstring for the full list (delimiter, encoding, quote/escape char,
#  header, skip_rows, footer_rows, comment_prefix, column_names,
#  column_mapping, dtype_mapping, null_values, trim_spaces, colspecs, ...).

SOURCE_FILE_CONFIG = None
#  Example - a plain CSV extract with a header row:
#  SOURCE_FILE_CONFIG = {
...
#  }
#
#  Example - a pipe-delimited feed with a 2-line metadata banner, a 1-line
#  footer and no header row of its own (irregular vendor layout):
#  SOURCE_FILE_CONFIG = {
...
#  --- Target: the DB table/view, same as Executor.py -----------------------
TARGET_SQL = None
#  Example:
#  TARGET_SQL = f"""
...
#  """
    ...
    # ########set SOURCE_FILE_CONFIG / TARGET_SQL to your real file/table.

def _build_demo_source_and_target():
    """Same built-in demo dataset as Executor.py's `_build_demo_source_and_target()`,
    with the source read from a real file (`sample_files/customers_source.csv`)
    instead of an in-memory list, so the demo also exercises File_Reader.py.
    It seeds the same realistic ETL bugs in the target -
    ...
    demo_source_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sample_files", "customers_source.csv")
    source_df = FileReader.read_file({
        "file_path": demo_source_path,
        "label": "source",
        "file_type": "csv",
        "dtype_mapping": {"customer_id": "bigint"},
    })
    ...
    target_df = spark.createDataFrame(target_rows, columns)

    key_col = KEY_COLUMNS[0]
    source_df.withColumn("keyColumn", source_df[key_col].cast("string")).createOrReplaceTempView("source")
    target_df.withColumn("keyColumn", target_df[key_col].cast("string")).createOrReplaceTempView("target")
...
#Columns to compare row-by-row between `source` (the file) and `target` (same name on both sides here; comparison_check() also accepts two different lists if your file and target column names diverge).
...
if SOURCE_FILE_CONFIG and TARGET_SQL:
    ...
    source_df = FileReader.read_file(SOURCE_FILE_CONFIG)
    source_df.createOrReplaceTempView("_source_raw")
    ...
    print("SOURCE_FILE_CONFIG / TARGET_SQL are not set - validating the built-in demo "
          "file/table instead.\n"
          "Set both above (or register your own 'source' / 'target' temp views) to "
          "validate a real file and table.")
...
# MAGIC %md
# MAGIC    Basic Checks
# MAGIC   - Source (File) and Target Count Check
...
# MAGIC
# MAGIC   Every check below is called completely unmodified from `Data_Validator.py` -
# MAGIC   the file was already turned into a `source` temp view above, so from this
# MAGIC   point on a file-backed source and a table-backed source look identical to
# MAGIC   every check.
# MAGIC
...
print("Source (File) - Target Count Check -->\n")
result = SourceCountCheck("source", "target")
data["Source (File) Vs Target Count Check"] = result
...
#  Driver-side files - toggle EXPORT_JSON / EXPORT_CSV / EXPORT_HTML
