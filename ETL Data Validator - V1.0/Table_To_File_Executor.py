...
# MAGIC %md
# MAGIC This is a **plug-and-play** Table -> File regression harness - the
# MAGIC same source-to-target validation as `Executor.py`, except the
# MAGIC **target** side comes from a flat file instead of a SQL query.
# MAGIC
...
# MAGIC   logic lives in this file - it only reads the target file into a
...
# MAGIC   2. **Locally / in CI** - `python Table_To_File_Executor.py` with
# MAGIC      `pyspark` installed (same local/CI story as `Executor.py`). With
# MAGIC      no target file configured it validates a small demo source table
# MAGIC      against the built-in demo file under
# MAGIC      `sample_files/customers_target.csv` so you can see every check
# MAGIC      working end to end.
...
snap_date = as_of_date   # the snapshot / partition date your source query filters on
print(f"********** Table -> File Validation run for {snap_date} **********")
...
job_id = "TABLE_TO_FILE_VALIDATION_JOB"
run_id = "local-run"
...
REPORT_TITLE = "Table-to-File Validation"  # heading on the HTML dashboard report
...
# DBTITLE 1,Source Query & Target File -- EDIT ME

#  --- Source: the DB table/view, same as Executor.py -----------------------
SOURCE_SQL = None
#  Example:
#  SOURCE_SQL = f"""
...
#  --- Target: a flat file, parsed via File_Reader.read_file() --------------
...
TARGET_FILE_CONFIG = None
#  Example - the nightly extract handed off to a downstream vendor:
#  TARGET_FILE_CONFIG = {
...
#  }
    ...
    # ########set SOURCE_SQL / TARGET_FILE_CONFIG to your real table/file.

def _build_demo_source_and_target():
    """Same built-in demo dataset as Executor.py's `_build_demo_source_and_target()`,
    with the target read from a real file (`sample_files/customers_target.csv`)
    instead of an in-memory list, so the demo also exercises File_Reader.py.
    It seeds the same realistic ETL bugs in the target file -
    ...
    source_df = spark.createDataFrame(source_rows, columns)

    demo_target_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sample_files", "customers_target.csv")
    target_df = FileReader.read_file({
        "file_path": demo_target_path,
        "label": "target",
        "file_type": "csv",
...
#Columns that must never be null/blank in `target` (the file).
MANDATORY_COLUMNS = ["first_name", "last_name", "email"]

#Columns to compare row-by-row between `source` and `target` (the file) (same name on both sides here; comparison_check() also accepts two different lists if your source and file column names diverge).
...
if SOURCE_SQL and TARGET_FILE_CONFIG:
    ...
    target_df = FileReader.read_file(TARGET_FILE_CONFIG)
    target_df.createOrReplaceTempView("_target_raw")
    spark.sql(f"SELECT {key_expr} AS keyColumn, * FROM _target_raw").createOrReplaceTempView("target")
else:
    print("SOURCE_SQL / TARGET_FILE_CONFIG are not set - validating a demo table "
          "against the built-in demo file instead.\n"
          "Set both above (or register your own 'source' / 'target' temp views) to "
          "validate a real table and file.")
...
# MAGIC %md
# MAGIC    Basic Checks
# MAGIC   - Source and Target (File) Count Check
...
# MAGIC   the file was already turned into a `target` temp view above, so from this
# MAGIC   point on a file-backed target and a table-backed target look identical to
# MAGIC   every check.
# MAGIC
...
print("Source - Target (File) Count Check -->\n")
result = SourceCountCheck("source", "target")
data["Source Vs Target (File) Count Check"] = result
